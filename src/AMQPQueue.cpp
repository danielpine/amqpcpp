...
{
	//// std::cout << "~AMQPQueue" << std::endl;
	//// std::cout << "~AMQPQueue" << pmessage << std::endl;
	if (pmessage)
		delete pmessage;
}
...
{
	//// std::cout << "sendDeclareCommand" << std::endl;
	if (!name.size())
...
{
	//// std::cout << "sendDeleteCommand" << std::endl;
...
{
	//// std::cout << "sendGetCommand" << std::endl;
	amqp_bytes_t queueByte = amqp_cstring_bytes(name.c_str());
...
//	}
	...
	while (1)
	{
		...
		// Add by Danielpine at 2019-03-18
		amqp_boolean_t redelivered = delivery->redelivered;

		pmessage->addHeader("redelivered", (uint8_t *)&redelivered);
...
{
	//// std::cout << "sendCancelCommand" << std::endl;
...
{
	//// std::cout << "getConsumerTag" << std::endl;
	return consumer_tag;
}
...
{
	//// std::cout << "sendAckCommand" << std::endl;
	amqp_basic_ack_t s;
